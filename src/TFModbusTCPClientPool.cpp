...
void TFModbusTCPClientPool::acquire(const char *host_name, uint16_t port,
                                    ...
                                    TFGenericTCPClientPoolDisconnectCallback &&disconnect_callback)
{
    ...
    for (size_t i = 0; i < TF_MODBUS_TCP_CLIENT_POOL_MAX_SLOT_COUNT; ++i) {
        TFModbusTCPClientPoolSlot *slot = slots[i];
        ...
        if (slot_host_name != nullptr && strcmp(slot_host_name, host_name) == 0 && slot->client.get_port() == port) {
            ...
            for (size_t k = 0; k < TF_MODBUS_TCP_CLIENT_POOL_MAX_HANDLE_COUNT; ++k) {
            ...
            TFModbusTCPClientPoolHandle *handle = new TFModbusTCPClientPoolHandle;
    ...
    TFModbusTCPClientPoolSlot *slot = new TFModbusTCPClientPoolSlot;
    slot->id = slot_id;
    ...
    [this, slot_index, slot_id](TFGenericTCPClientConnectResult result, int error_number) {
        for (size_t i = 0; i < TF_MODBUS_TCP_CLIENT_POOL_MAX_HANDLE_COUNT; ++i) {
            TFModbusTCPClientPoolSlot *slot = slots[slot_index];

            if (slot == nullptr || slot->id != slot_id) {
                return; // slot got freed or reused
            }

            TFModbusTCPClientPoolHandle *handle = slot->handles[i];
...
void TFModbusTCPClientPool::release(TFModbusTCPClientPoolHandle *handle)
{
    handle->pending_release = true;
}

void TFModbusTCPClientPool::tick()
{
    for (size_t i = 0; i < TF_MODBUS_TCP_CLIENT_POOL_MAX_SLOT_COUNT; ++i) {
        TFModbusTCPClientPoolSlot *slot = slots[i];
        ...
        for (size_t k = 0; k < TF_MODBUS_TCP_CLIENT_POOL_MAX_HANDLE_COUNT; ++k) {
            TFModbusTCPClientPoolHandle *handle = slot->handles[k];
